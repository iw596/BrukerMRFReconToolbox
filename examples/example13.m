...
% Parameters
matrixSize = [128, 128]; % Matrix size
undersamplingRate = 40; % Desired undersampling rate
numTimePoints = 500; % Total time points in the MRF signal train
rank = 6; % Number of low-rank coefficients to retain

% Step 1: Generate sampling masks
samplingMasks = zeros([matrixSize, numTimePoints]);
for t = 1:numTimePoints
    samplingMasks(:, :, t) = generatePoissonDiscPattern(matrixSize, undersamplingRate);
end

% Step 2: Apply sampling masks to k-space data
undersampled_kspace = zeros(size(kspace));
for t = 1:numTimePoints
    undersampled_kspace(:, :, t) = kspace(:, :, t) .* samplingMasks(:, :, t);
end
save('undersampled_kspace.mat', 'undersampled_kspace');
disp('Pseudorandom Poisson-disc undersampling applied and saved.');

% Step 3: Perform iterative low-rank reconstruction
coefficientImages = iterativeLowRankReconstruction(undersampled_kspace, rank);
