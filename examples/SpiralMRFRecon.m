...
%% Load and generate B1 map
% pth = "C:\Users\kpqv532\OneDrive - University of Leeds\20250718_133850_MRF_Phantom_3dMRFDev_v2_18072025_1_36\14";
% params_B1Map = LoadBrukerData(pth);
% nextMultiple = 128 * ceil((params_B1Map.NCol) / 128);
% padding = nextMultiple - params_B1Map.NCol;
% data = params_B1Map.data;
% data = reshape(data,[nextMultiple,2,params_B1Map.NLin,params_B1Map.NPar]);
% data = data(1:params_B1Map.NCol,:,:);
% data = reshape(data,[params_B1Map.NCol,2,params_B1Map.NLin,params_B1Map.NPar]);
% data = permute(data,[1 3 4 2]);
% imgs = ifftcn(data,[1 2 3]);
% AFIB1Map = FitAFIB1(imgs,60,20,20*5);
% AFIB1MapResized = imresize3(AFIB1Map,[params.NCol params.NLin params.NPar]);



img = nufft_adj(area.*(data), nufft_st);  % adjoint (gridding);
figure; plot(squeeze(abs(img(params.NCol/2,params.NCol/2,:))))
img = flipdim(img,1);
img = flipdim(img,1);

% Reshape data to add third spatial dimension

figure; imshow(abs(img(:,:,700)),[]);
...
data = reshape(data,[params.NCol params.NLin 1 params.NInv]);
fclose(fid);
...
fid = fopen(pth + '\pdata\1\2dseq');
data = fread(fid,"int16");
data = reshape(data,[params.NCol params.NLin 1 params.NEcho]);
data = data./max(data,[],'all');
fclose(fid);
...
save("Results/MatchingWithInversion","T1Map_MRF","T2Map_MRF","T1FitResults","T2FitResults");
...
degree = 1;
p = polyfit(RefT2_meanValues(1:4),MRFT2_meanValues(1:4),degree);

% Evaluate the fitted polynomial p and plot:
f = polyval(p,RefT2_meanValues(1:4));
eqn = poly_equation(p); % polynomial equation (string)
Rsquared = my_Rsquared_coeff(MRFT2_meanValues(1:4),f); % correlation coefficient

figure(65);
plot(RefT2_meanValues(1:4),MRFT2_meanValues(1:4),'x',RefT2_meanValues(1:4),f,'--',LineWidth=2)
legend('data',eqn)
