...
gridSize = [128, 128];
numTimePoints = 1980;
accelerationFactor = 6; % Update to 8x
centralCoverage = 0.12; % 15% central coverage

% Generate sampling masks

samplingMasks = generatePoissonMasks16(gridSize, numTimePoints, accelerationFactor, centralCoverage);

% Visualize a few sampling masks
figure;
for i = 1:6
    subplot(2, 3, i);
    imshow(samplingMasks(:, :, i), []);
    title(['Sampling Mask (Time Point ' num2str(i) ')']);
end

% Visualize one example mask
figure;
imagesc(samplingMasks(:, :, 1)); colormap gray; axis equal; colorbar;
title('Sampling Mask for First Time Point');

% % Load fully sampled k-space data
% load('full_kspace.mat'); % Replace with your file name

% Initialize undersampled k-space
undersampled_kspace = zeros(size(kspace));

% Apply masks to each time point
for t = 1:numTimePoints
    undersampled_kspace(:, :, t) = kspace(:, :, t) .* samplingMasks(:, :, t);
end

% Save undersampled data
save('undersampled_kspace.mat', 'undersampled_kspace', '-v7.3');

disp(['Total sampled points per time point: ', num2str(mean(sum(sum(samplingMasks, 1), 2)))]);
figure;
imagesc(mean(samplingMasks, 3)); colormap gray; colorbar;
title('Mean Sampling Mask Across Time Points');

%% Step 2: Reconstruction
% Reconstruct spatial domain images from k-space using IFT
imgs = ifft2(ifftshift(kspace)); % Fully sampled reconstruction
undersampled_imgs = ifft2(ifftshift(undersampled_kspace)); % Undersampled reconstruction % Undersampled reconstruction
...
sliceIdx = 64; % Adjust as needed
figure;
subplot(1, 2, 1); imagesc(abs(imgs(:, :, sliceIdx))); axis image; colorbar;
title('Fully Sampled Slice');
subplot(1, 2, 2); imagesc(abs(undersampled_imgs(:, :, sliceIdx))); axis image; colorbar;
title('Undersampled Slice');
...
disp(['  Fully Sampled - T1: ' num2str(T1_maps{1}(x, y)) ', T2: ' num2str(T2_maps{1}(x, y))]);
disp(['  Undersampled  - T1: ' num2str(T1_maps{2}(x, y)) ', T2: ' num2str(T2_maps{2}(x, y))]);
...
assert(~isempty(T1_map) && all(size(T1_map) == [Nx, Ny]), 'T1 map is invalid.');
assert(~isempty(T2_map) && all(size(T2_map) == [Nx, Ny]), 'T2 map is invalid.');

figure;
imagesc(squeeze(mean(samplingMasks, 3))); title('Mean Sampling Mask');
figure;
imagesc(abs(imgs(:, :, 1))); hold on;
contour(mask, [0.5, 0.5], 'r', 'LineWidth', 1.5); % Overlay the mask
title('Reconstructed Image with Phantom Mask');

% Visualize the k-space magnitude
figure;
imagesc(log(abs(kspace(:, :, 1)))); colormap gray; axis image;
title('k-Space Magnitude (Time Point 1)'); colorbar;

% Visualize the k-space phase
figure;
imagesc(angle(kspace(:, :, 1))); colormap hsv; axis image;
title('k-Space Phase (Time Point 1)'); colorbar;
% Visualize magnitude of fully sampled k-space
figure;
imagesc(log(abs(kspace(:,:,1)) + eps)); colormap('gray'); colorbar;
title('Fully Sampled k-Space Magnitude');

% Visualize magnitude of undersampled k-space
figure;
imagesc(log(abs(undersampled_kspace(:,:,1)) + eps)); colormap('gray'); colorbar;
title('Undersampled k-Space Magnitude');
figure;
imagesc(log(abs(kspace(:,:,1)))); colormap gray; axis image; colorbar;
title('Fully Sampled k-Space (Log Magnitude)');
