...
% Ramp up max peaks
maxFA = round(linspace(20,75,4));
maxFA(end+1:end+length(maxFA)-1) = maxFA(end-1:-1:1);
lobeGap  =10;

faFilePath = "C:\Users\kpqv532\OneDrive - University of Leeds\MRF_FA_Patterns\MRFFAPattern.txt";
FAPattern = GenerateFAPattern(NLobes,NPoints,minFA,maxFA,lobeGap,0);


%% Now generate TR Pattern
persistence = 0.6; % Moderate persistence for smoother output
octaves = 70;      % Number of octaves
TRMin = 10e-3;      % Desired minimum value
TRMax = 25e-3;       % Desired maximum value
