...
kspace_size = [128, 128];
num_timepoints = 1980;
acceleration_factor = 4;  % 4x undersampling
central_fraction = 0.3;   % Fully sample ~30% of k-space center

% Generate sampling masks
sampling_masks = generate_poisson_masks(kspace_size, num_timepoints, acceleration_factor, central_fraction);

% Visualize a few sampling masks
figure;
for i = 1:6
    subplot(2, 3, i);
    imshow(sampling_masks(:, :, i), []);
...
for t = 1:num_timepoints
    undersampled_kspace(:, :, t) = kspace(:, :, t) .* sampling_masks(:, :, t);
end
figure(5); imagesc(log(abs(undersampled_kspace(:,:,1))))
