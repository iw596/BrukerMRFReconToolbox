...
for t = 2:num_timepoints
    if isequal(sampling_masks(:, :, t), sampling_masks(:, :, t-1))
...
tp1 = randi([1, num_timepoints]);
tp2 = randi([1, num_timepoints]);

figure;
subplot(1, 2, 1);
imshow(sampling_masks(:, :, tp1), []);
title(['Mask for Time Point ' num2str(tp1)]);

subplot(1, 2, 2);
imshow(sampling_masks(:, :, tp2), []);
title(['Mask for Time Point ' num2str(tp2)]);

% Display difference between two masks
difference = sampling_masks(:, :, tp1) - sampling_masks(:, :, tp2);
figure;
imshow(difference, []);
...
figure;
imshow(mat2gray(sampling_masks)); % Normalize the mask to the range [0, 1]
title('Sampling Mask');
...
[rows, cols] = find(sampling_masks); % Coordinates of sampled points
...
figure;
heatmap(sampling_masks, 'Colormap', gray, 'ColorLimits', [0, 1]);
title('Sampling Mask');


figure;
imagesc(sampling_masks); % Visualize the sampling mask
...
sampling_mask_single = sampling_masks(:, :, 1);
...
for t = 1:5:1980 % Adjust the step size (1:5) as needed
    sampling_mask_single = sampling_masks(:, :, t);
    figure;
    imagesc(sampling_mask_single);
    colormap gray;
    colorbar;
