% function [samplingMask, matchingMask] = poissonDiskCentralBias4(gridSize, undersamplingFactor, radius, biasFactor)
%     % Total target points
...
%     densityWeights = densityWeights + 0.1; % Allow peripheral sampling
%     densityWeights = densityWeights / sum(densityWeights); % Normalize
% 
%     % Dynamic radius based on distance from center
%     radiusScaling = 1 + distances / max(distances);
%     dynamicRadius = radius ./ radiusScaling;
% 
%     % Sampling logic
%     while size(sampledPoints, 1) < totalPoints && ~isempty(gridPoints)
%         % Sample a point based on probabilities
%         cumulativeProbs = cumsum(densityWeights);
%         randomValue = rand * cumulativeProbs(end);
%         candidateIdx = find(cumulativeProbs >= randomValue, 1);
%         candidate = gridPoints(candidateIdx, :);
% 
%         % Compute distances with dynamic radius
%         if isempty(sampledPoints) || all(vecnorm(sampledPoints - candidate, 2, 2) >= dynamicRadius(candidateIdx))
...
%         densityWeights(candidateIdx) = [];
%         densityWeights = densityWeights / sum(densityWeights);
%     end
% 
%     % Matching mask creation (circular region)
%     [X, Y] = ndgrid(1:gridSize(1), 1:gridSize(2));
%     distancesFromCenter = sqrt((X - center(1)).^2 + (Y - center(2)).^2);
%     matchingMask(distancesFromCenter <= gridSize(1) / 2) = 1; % Circular region
% 
%     % Display results
...
%     % Visualize masks
%     figure;
%     subplot(1, 2, 1); imagesc(samplingMask); axis image; colorbar;
%     title('Sampling Mask');
%     subplot(1, 2, 2); imagesc(matchingMask); axis image; colorbar;
%     title('Matching Mask');
% end
