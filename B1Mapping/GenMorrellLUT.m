...
function LUT = GenMorrellLUT(alpha, omega)
    ...
    MyPlus = ((2.*alphaMesh.*sin(beta))./beta.^3).*(omegaMesh.^2.*cos(beta).^2 + alpha.^2.*cos(beta));
    ...
    MyNeg =  -1 .* ((2.*alphaMesh.*sin(beta))./beta.^3).*(omegaMesh.^2.*cos(beta).^2 + alpha.^2.*cos(beta));
   ...
   % phaseDiff = angle(MxyPos) - angle(MxyNeg);
    %phaseDiff = angle(MxyPos.*conj(MxyNeg));
    %phaseDiff = atan2(imag(MxyPos./MxyNeg),real(MxyPos./MxyNeg));
phaseDiff =  atan2(MyNeg,MxNeg) - atan2(MyPlus,MxPlus);
