function M_Echo = FISPSimulation(SeqParams,TissueParams,NIso)
    ...
    TE = SeqParams.TE;
    TR = SeqParams.TR;
    %Create spin distribution between -4pi and 4pi
    phi = linspace(-4*pi,4*pi,NIso);
    M = zeros([3,NIso]);
   ...
   % 
    ...
    %     M(2,:) = 1;
    %     Mtmp = Rot(slice_profile(is)*pi*round(cos(pi))) * real(M);
    %     M = Mtmp + 1i*imag(M);
    %     %Relaxation
    %     M(1,:) = M(1,:) .* exp(-TI./T2);
